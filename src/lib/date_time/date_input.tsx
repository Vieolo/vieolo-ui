...
export default function DateInput(props: {
    ...
    return <InputSet
        ...
        onChange={v => {
            ...
            if (v.length > 0 && v[v.length - 1] !== '/' && isNaN(+v[v.length - 1])) return;

            if (v[v.length -1] === '/' && [1, 2, 4, 5, 7, 8, 9, 10].includes(v.length)) return;

            //  Adding "/" if the user has typed 2 characters
            if (props.value.length === 1 && v.length === 2) v += "/";

            //  Adding "/" if the user has typed 5 characters
            if (props.value.length === 4 && v.length === 5) v += "/";
            ...
            if (props.value.length === 5 && v.length === 6 && v[5] !== "/") v = `${v.substring(0, 5)}/${v[5]}`

            // Preventing the user to enter more than two "/"
            if (props.value.length === 3 && v.length === 4 && v[2] === "/" && v[3] === "/") return;
            if (props.value.length === 6 && v.length === 7 && v[5] === "/" && v[6] === "/") return;
